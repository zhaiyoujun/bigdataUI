...
public class ShellService {
	public String[][] getClusterStatus(String[] nodeSet, String[] ipSet, ArrayList<String> pingCmdSet) throws IOException, InterruptedException {
		Process process;
		int ipCount = ipSet.length;
		String[][] clusterStatus = new String[ipCount][3];
		
		for (int i = 0; i < ipCount; i++) {
			clusterStatus[i][0] = nodeSet[i];			
			clusterStatus[i][1] = ipSet[i];			
			process = Runtime.getRuntime().exec(pingCmdSet.get(i));			
			int status = process.waitFor();
			
			if (status == 0) {
				clusterStatus[i][2] = "Living";
			}else {
				clusterStatus[i][2] = "Die";
			}			
		}
	...
	public String getClusterStatusJS(String[][] clusterStatus) throws IOException, InterruptedException {
	    ...
	    for (int i = 0; i < ipCount; i++) {
	    	...
	    	for (int j = 0; j < clusterStatus[i].length; j++) {
	...
	public HashMap<String, Integer> getStatisticsResult(String[][] clusterStatus) {
		...
		for (int i = 0; i < clusterStatus.length; i++) {
			if (clusterStatus[i][2].equals("Living")) {
